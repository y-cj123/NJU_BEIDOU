...
每次数据未收完整保存的临时文件均未删除。重新启动主要手动删除临时文件。
...
-----------------------------------------------------------------------------------------------------------------
新增串口（北斗）准备就绪标志位，当需要使用北斗服务时先计算本次数据发送时间与上次发送时间之间的时长，低于北斗发送间隔（60s），
就要返回未就绪的状态指示：
1、Socket_recieve.c:
	收到server发过来需要转发的数据：转发前判断
	与服务端交互报文格式：00 ff 00 11(这一位表示还需要等待多少s，0~60，例如11：代表还剩17s)
2、BD_Recive.c
	write();
	在向串口写入之前作状态判断